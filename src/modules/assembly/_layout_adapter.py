...
class LayoutAdapterMixin(AssemblyAdapterCommon):
    ...
    def from_layout_output(cls, raw: Any) -> AssemblyResult:
        ...
        return AssemblyResult(
            ...
            document=AssembledDocument(
                ...
                metadata=cls._extract_layout_document_metadata(raw),
                raw=raw,
            ),
    ...
    def _extract_object_refs_from_elements(
        ...
        for element in elements:
            if element.kind == "table":
                # table crop 경로 같은 연결 힌트는 metadata에 남겨 다음 단계가 활용하게 한다.
                ...
                figure_refs.append(
                    FigureRef(
                        ...
                        source_block_ids=[element.id],
                        metadata={"source": "layout_element", **element.metadata, **figure_metadata},
                        raw=element.raw,
                    )
                )
                ...
                note_refs.append(
                    NoteRef(
                        ...
                        text=element.text,
                        source_block_ids=[element.id],
                        metadata={"source": "layout_element", **element.metadata},
                        raw=element.raw,
                    )
                )
    ...
    @classmethod
    def _extract_layout_document_metadata(cls, raw: Any) -> Dict[str, Any]:
        """문서 단위 상위 메타데이터를 그대로 보존한다."""
        ...
        metadata = cls._extract_metadata(
            raw,
            {
                *cls.PAGE_LIST_KEYS,
                *cls.PAGE_STATS_KEYS,
                *cls.FIGURE_ASSET_KEYS,
                *cls.TABLE_CONTAINER_KEYS,
            },
        )

        for key in cls.DOCUMENT_METADATA_KEYS:
    ...
    def _build_element(
        ...
        payload = raw if isinstance(raw, dict) else {"text": raw}  # text block fallback
        ...
        raw_element_id = cls._normalize_str(cls._pick_first(payload, cls.ELEMENT_ID_KEYS))
        metadata = cls._extract_metadata(
            payload,
            {
                *cls.ELEMENT_ID_KEYS,
        ...
        if raw_element_id is None:
            ...
            # 실제 layout 샘플은 페이지별 정수 id를 재사용하므로 내부 id는 페이지/종류를 붙여 고정한다.
